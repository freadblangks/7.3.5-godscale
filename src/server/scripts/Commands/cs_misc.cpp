...
#define SPELL_UNSTUCK_VISUAL 2683
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        Item* item = playerTarget->StoreNewItem(dest, itemId, true, GenerateItemRandomPropertyId(itemId), GuidSet(), 0, bonusListIDs, false, transmogID);
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            if (itr->second.GetItemSet() == itemSetId)
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
                    ...
                    //Item* item = playerTarget->StoreNewItem(dest, itr->second.GetId(), true, GenerateItemRandomPropertyId(itr->second.GetId()), GuidSet(), 0, bonusListIDs, false, transmogID);
